...
#include "Navigation.h" // Nécessaire pour WorldToGrid
#include <iostream>
#include <cmath>
...
Player::Player()
    ...
    frameTime(0.f),
    animationSpeed(0.1f),
    currentDirection(Direction::Right) // Direction par défaut
{
    // Chargement de la texture (Chemin adapté selon votre structure)
    ...
    frameSize.x = static_cast<int>(texture.getSize().x) / 8; // 8 frames
...
void Player::update(float deltaTime, const Background& background)
{
    ...
    // Gestion des entrées SFML 3.0 / Clavier
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Z)) {
        ...
        sprite.setScale(sf::Vector2f(-currentScaleVal, currentScaleVal)); // Miroir
        sprite.setRotation(sf::degrees(0.f));
    }
    else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::D)) {
        movement.x += 1.f;
        currentDirection = Direction::Right;
        sprite.setScale(sf::Vector2f(currentScaleVal, currentScaleVal)); // Normal
        sprite.setRotation(sf::degrees(0.f));
    }
    ...
    if (movement.x != 0.f || movement.y != 0.f)
    {
        ...
        // Collisions simples aux 4 coins de la hitbox
